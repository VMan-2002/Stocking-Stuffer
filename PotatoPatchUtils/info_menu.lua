    ...
    if loc then
        ...
        if target then
            ...
            for i, v in ipairs(target.text) do
                info_nodes.nodes[1].nodes[#info_nodes.nodes[1].nodes + 1] = create_text_box({ loc_target = v, text_parsed =
                                                ...
                                                nodes = {
                                                    {
                                                        ...
                                                        nodes = {
                                                            ...
                                                            create_toggle({ label = localize(
                                                            "PotatoPatchUtils_first_time_disable"), ref_table = G
                                                            .PROFILES[G.SETTINGS.profile].first_time_disable, ref_value =
                                                            menu_type, callback = function() end }) or nil,
                                                        }
                                                    },
                                                }
                                            },
    ...
    if loc and loc.text and #loc.text > 1 then
        local pages = {
            ...
            nodes = {
                {
                    n = G.UIT.C,
                    config = { align = "cm", minw = 0.5, minh = 0.3, maxh = 0.5, padding = 0.1, r = 0.1, hover = true, colour = args.page_colour or G.C.BLACK, shadow = true, button = "potato_patch_utils_info_menu_switch_page", page_change = -1, menu_type = menu_type, page = page, max_page = (#(loc.text or {}) or 1), back_func = back_func, no_first_time = no_first_time, args = args },
                    nodes = {
                        {
                            n = G.UIT.R,
                            config = { align = "cm", padding = 0.05 },
